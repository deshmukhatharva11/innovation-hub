...
app.post('/api/cms/content', (req, res) => {
  try {
    console.log('Creating content with data:', JSON.stringify(req.body, null, 2));
...
app.post('/api/cms/notifications', (req, res) => {
  try {
    console.log('Creating notification with data:', JSON.stringify(req.body, null, 2));
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        pagination: {
          total: Object.keys(defaultTemplates).length,
          page: 1,
          limit: 10,
          totalPages: 1
        }
...
app.get('/health', (req, res) => {
  res.json({
    status: 'OK',
    message: 'Working CMS Server is running',
...
app.listen(PORT, () => {
  console.log('🚀 Working CMS Server running on port', PORT);
